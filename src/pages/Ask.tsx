...
import { CodeEditor, Editor, Question } from '../containers/Ask';
...
const Ask: React.SFC<IAskProps> = ({ initAction }) => {
  ...
  const [state, setState] = useState<IAskState>({
    contents: '',
  });
  const handleContents = useCallback((content: string) => {
    setState(prev => ({
      ...prev,
      contents: content,
    }));
